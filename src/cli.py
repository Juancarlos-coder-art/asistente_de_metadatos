...
def save_output(state: MetadataState, path: str | None = None):
    """
    Guarda el JSON de salida SIEMPRE dentro de <raiz_proyecto>/datasets,
    independientemente de desde dónde ejecutes el script.
    """
    # carpeta del archivo actual
    this_dir = Path(__file__).resolve().parent  # .../asistente_de_metadatos (ajusta si este archivo está en otra ruta)

    # si este archivo está en, por ejemplo, src/ o assistant/, sube al raíz del repo:
    #   raiz = this_dir.parent           # si este .py está en 'assistant/'
    #   raiz = this_dir.parent.parent    # si está dos niveles por debajo
    #
    # En tu caso (por lo que muestras), este archivo parece estar en la raíz del repo o en /assistant.
    # Ajusta UNA de las siguientes líneas según sea tu estructura real:

    raiz = this_dir  # si este .py está en la raíz del proyecto
    # raiz = this_dir.parent  # descomenta esta si el .py está en 'assistant/' y el repo raíz está un nivel arriba

    datasets_dir = raiz / "datasets"
    datasets_dir.mkdir(parents=True, exist_ok=True)

    if path is None:
        path = datasets_dir / "metadata_output.json"
    else:
        # Si te pasan un path relativo, hazlo relativo a datasets/
        path = Path(path)
